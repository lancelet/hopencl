...
-- TODO: investigate foldr: is it an appropriate use?
...
class Const t u | t -> u where
    value :: t -> u

data BitSet t where
    BitSet :: (Const t u, Bits u, Integral u) => u -> BitSet t

instance (Const t u, Enum t, Bounded t, Show t) => Show (BitSet t) where
    show b = "bitSet " ++ show (allSetIn b)
...
-- TODO: Split these into a separate file?  Also should probably include
--       someone besides AMD.
amdVendorStrings :: [String]
...
data DeviceFPConfig
    = FPDenorm | FPInfNAN | FPRoundToNearest | FPRoundToZero | FPRoundToInf
    | FPFMA | FPSoftFloat
    deriving (Bounded, Enum, Eq, Ord, Show)

instance Const DeviceFPConfig CLBitField where
    value FPDenorm         = 1
...
data DeviceMemCacheType
    = NoCache | ReadOnlyCache | ReadWriteCache
    deriving (Bounded, Enum, Eq, Ord, Show)

instance Const DeviceMemCacheType CLUInt where
    value NoCache        = 0
...
instance Const DeviceLocalMemType CLUInt where
    value Local  = 1
    value Global = 2

data DeviceExecutionCapabilities
    = CLKernel | NativeKernel
    deriving (Bounded, Enum, Eq, Ord, Show)

instance Const DeviceExecutionCapabilities CLBitField where
    value CLKernel     = 1
    value NativeKernel = 1 `shiftL` 1

-- And here.. we.. go!
data DeviceInfo t where
    DeviceType                     :: DeviceInfo (BitSet DeviceType)
...
instance Const (DeviceInfo t) CLUInt where
    value DeviceType                        = 0x1000
...
-- At this point, I'm treating context property lists as being basically
-- write-only.  I'd like things to be (mostly) strongly typed on the way
-- in---that is, you can't specify arbitrary values as context properties or
-- specify properties and values that don't match---but I'm not making any
-- attempt to make it possible to read back out of a list of context properties
-- once it's created.  It might actually be possible assuming you knew what
-- properties to look for and were willing to do a fair amount of
-- 'unsafeCoerce'ing, but I'm not doing that now.

-- The set of context properties seems to be a fairly extensible thing---for
-- instance, the CL/GL interop stuff adds to the list of context properties.
-- So, rather than hard-code the set of possible properties, I'm using a class
-- to identify those types that could be context properties.  The important
-- thing about this class is the Const precondition; otherwise, it just serves
-- as a type-level tag.  Additionally, this will only work for types 'u' that
-- can be 'unsafeCoerce'd into 'CInt's, but I'm leaving that up to the types in
-- 'ContextProperty' to ensure.

class Const (t u) CLContextProperty => ContextProperty t u

-- Rather than treat a list of context properties using some kind of existential
-- pair 'exists t u. ContextProperty t u => (t u, t)', I'm using the underlying
-- representation of a list of 'CInt's.  This makes it fantastically easy to
-- pass a list of context properties to the runtime, or get one back from the
-- runtime---I just have to do the normal array marshalling stuff.  It makes it
-- less possible to actually read property-value pairs back out of a list of
-- context properties, but I don't see either a really good way to do that, or a
-- really good reason.
...
pushContextProperty :: ContextProperty t u
                    => t u
                    -> u
                    -> ContextProperties
                    -> ContextProperties
...
data CoreContextProperty t where
    ContextPlatform :: CoreContextProperty Platform

instance Const (CoreContextProperty t) CLContextProperty where
    value ContextPlatform = 0x1084

instance ContextProperty CoreContextProperty t

-- And that's all for context properties for now.  This interface works, but it
-- has limitations.
...
data CommandQueueProperty
    = OutOfOrderExecution | Profiling
    deriving (Bounded, Enum, Eq, Ord, Show)

instance Const CommandQueueProperty CLCommandQueueProperties where
    value OutOfOrderExecution = 1
    value Profiling           = 1 `shiftL` 1

data CommandQueueInfo t where
    QueueContext        :: CommandQueueInfo Context
...
instance Const (CommandQueueInfo t) CLCommandQueueInfo where
    value QueueContext        = 0x1090
...
data MemFlag
    ...
    | UseHostPtr | AllocHostPtr | CopyHostPtr
    deriving (Bounded, Enum, Eq, Ord, Show)

instance Const MemFlag CLMemFlags where
    value ReadWrite    = 1 `shiftL` 0
...
instance Const BufferCreateType CLBufferCreateType where
    value CreateTypeRegion = 0x1220
...
data CLImageFormat
    = CLImageFormat
    { imageChannelOrder    :: CLChannelOrder
    , imageChannelDataType :: CLChannelType
    } deriving (Eq, Show)
...
instance Const ChannelOrder CLChannelOrder where
    value R         = 0x10B0
    value A         = 0x10B1
    value RG        = 0x10B2
    value RA        = 0x10B3
...
instance Const ChannelType CLChannelType where
    value ChanTypeSNormInt8      = 0x10D0
...
data MemObjectType
    = MemObjectBuffer | MemObjectImage2D | MemObjectImage3D
    deriving (Bounded, Enum, Eq, Ord, Show)

instance Const MemObjectType CLMemObjectType where
    value MemObjectBuffer  = 0x10F0
    value MemObjectImage2D = 0x10F1
    value MemObjectImage3D = 0x10F2

data ImageInfo t where
    ImageFormat      :: ImageInfo CLImageFormat
...
instance Const (ImageInfo t) CLImageInfo where
    value ImageFormat      = 0x1110
...
data MemInfo t where
    MemType                :: MemInfo MemObjectType
...
instance Const (MemInfo t) CLMemInfo where
    value MemType                = 0x1100
...
data AddressingMode
    = AddressNone | AddressClampToEdge | AddressClamp | AddressRepeat
    | AddressMirroredRepeat
    deriving (Bounded, Enum, Eq, Ord, Show)

instance Const (AddressingMode) CLAddressingMode where
    value AddressNone           = 0x1130
...
data FilterMode
    = FilterNearest | FilterLinear deriving (Bounded, Enum, Eq, Ord, Show)

instance Const FilterMode CLFilterMode where
    value FilterNearest = 0x1140
...
data SamplerInfo t where
    SamplerReferenceCount   :: SamplerInfo CLUInt
...
instance Const (SamplerInfo t) CLSamplerInfo where
    value SamplerReferenceCount   = 0x1150
...
data ProgramInfo t where
    ProgramReferenceCount :: ProgramInfo CLUInt
...
instance Const (ProgramInfo t) CLProgramInfo where
    value ProgramReferenceCount = 0x1160
...
data BuildStatus
    = BuildNone | BuildError | BuildSuccess | BuildInProgress
    deriving (Bounded, Enum, Eq, Ord, Show)
type CLBuildStatus = CLInt

instance Const BuildStatus CLBuildStatus where
    value BuildSuccess    = 0
    value BuildNone       = -1
    value BuildError      = -2
    value BuildInProgress = -3

data ProgramBuildInfo t where
    ProgramBuildStatus  :: ProgramBuildInfo BuildStatus
    ProgramBuildOptions :: ProgramBuildInfo String
    ProgramBuildLog     :: ProgramBuildInfo String

instance Const (ProgramBuildInfo t) CLProgramBuildInfo where
    value ProgramBuildStatus  = 0x1181
...
data KernelInfo t where
    KernelFunctionName   :: KernelInfo String
...
instance Const (KernelInfo t) CLKernelInfo where
    value KernelFunctionName   = 0x1190
    value KernelNumArgs        = 0x1191
    value KernelReferenceCount = 0x1192
    value KernelContext        = 0x1193
    value KernelProgram        = 0x1194

data KernelWorkGroupInfo t where
    KernelWorkGroupSize                  :: KernelWorkGroupInfo SizeT
...
instance Const (KernelWorkGroupInfo t) CLKernelWorkGroupInfo where
    value KernelWorkGroupSize                  = 0x11B0
...
instance Const CommandType CLCommandType where
    value CommandNDRangeKernel     = 0x11F0
...
data EventStatus
    = Complete | Running | Submitted | Queued | Errored Int deriving (Eq, Show)

instance Const EventStatus CLInt where
    value Complete    = 0
    value Running     = 1
    value Submitted   = 2
    value Queued      = 3
...
eventStatusFromCLInt
    n | n < 0     = Errored (fromIntegral n)
      | otherwise =
            error $ "Uninterpreted event status in eventStatusFromCLInt: " ++
                    show n

data EventInfo t where
    EventCommandQueue           :: EventInfo CommandQueue
...
instance Const (EventInfo t) CLEventInfo where
    value EventCommandQueue           = 0x11D0
...
data ProfilingInfo t where
    ProfilingCommandQueued :: ProfilingInfo CLULong
...
instance Const (ProfilingInfo t) CLProfilingInfo where
    value ProfilingCommandQueued = 0x1280
